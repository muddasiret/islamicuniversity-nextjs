...
const NewsCard = ({ newsDetails }) => {
  ...
  return (
    <Link href={`/news/${slug}`} passHref={true}>
      <div className="mt-5 text-left bg-primaryblue cursor-pointer shadow-lg group card-zoom hover:shadow-2xl">
        <p className="text-left text-md uppercase text-lightyellow px-4 pt-3 font-bold ">
          {category}
        </p>
        <div
        ...
        />
        <h2 className="mt-56 text-left text-md uppercase text-skyblue px-4 pb-2 pt-2 font-bold leading-6">
          {title}
        </h2>
        <p className="text-left px-4 mb-5 text-xs text-white">
          {subtitleFormatted}
        </p>
